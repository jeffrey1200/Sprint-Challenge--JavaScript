...
const tyrannosaurus = {
  ...
  roar: () => console.log("RAWERSRARARWERSARARARRRR!")
}
...
universities.sort((value,value2) => (value > value2) ? 1 : -1)
...
const contactInfo = [];
for(let i = 0; i < graduates.length; i++){
  let result = `${graduates[i].first_name} ${graduates[i].email}`
  contactInfo.push(result)
}
...
*/
const displayNames = [];
let process = zooAnimals.forEach((animalScientic) => displayNames.push(`Name: ${animalScientic.animal_name}, Scientific: ${animalScientic.scientific_name}`))
...
let mapping = zooAnimals.map((names) =>lowCaseAnimalNames.push(names.animal_name.toLowerCase()))
...
let filtered = zooAnimals.filter((currentValue) => currentValue.population < 5)
...
let total = zooAnimals.reduce((accumulator, currentValue) => accumulator + currentValue.population, 0)
